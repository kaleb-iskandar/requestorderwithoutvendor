...
/**
 *   	\file       requestpurchaseorderline_list.php
 *		\ingroup    requestorderwithoutvendor
 *		\brief      List page for requestpurchaseorderline
 */
...
// load requestorderwithoutvendor libraries
require_once __DIR__.'/class/requestpurchaseorderline.class.php';
...
$hookmanager->initHooks(array('requestpurchaseorderlinelist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('requestorderwithoutvendor', 'requestpurchaseorderline', 'write');
	$permissiontodelete = $user->hasRight('requestorderwithoutvendor', 'requestpurchaseorderline', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("requestorderwithoutvendor")) {
	accessforbidden('Module requestorderwithoutvendor not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'RequestPurchaseOrderLine';
	$objectlabel = 'RequestPurchaseOrderLine';
	$uploaddir = $conf->requestorderwithoutvendor->dir_output;
...
//$help_url = "EN:Module_RequestPurchaseOrderLine|FR:Module_RequestPurchaseOrderLine_FR|ES:Módulo_RequestPurchaseOrderLine";
$help_url = '';
$title = $langs->trans("RequestPurchaseOrderLines");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/requestorderwithoutvendor/requestpurchaseorderline_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/requestorderwithoutvendor/requestpurchaseorderline_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendRequestPurchaseOrderLineRef";
$modelmail = "requestpurchaseorderline";
$objecttmp = new RequestPurchaseOrderLine($db);
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_requestorderwithoutvendor', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
