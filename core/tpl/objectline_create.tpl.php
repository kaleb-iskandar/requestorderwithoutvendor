...
if ($nolinesbefore) {
	...
	<tr class="liste_titre<?php echo (($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_add_') ?> nodrag nodrop">
		...
		<td class="linecoldescription minwidth400imp">
			<div id="add"></div><span class="hideonsmartphone"><?php echo $langs->trans('AddPurchaseRequestProductLine'); ?></span>
		</td>
		<?php
		...
		if($object->status!=RequestPurchaseOrder::STATUS_DRAFT && $object->status!=RequestPurchaseOrder::STATUS_REJECTED){
			?>
...
<tr class="pair nodrag nodrop nohoverpair<?php echo ($nolinesbefore || $object->element == 'contrat') ? '' : ' liste_titre_create'; ?>">
	...
	<td class="nobottom linecoldescription minwidth400imp">
		...
		if (isModEnabled("product") || isModEnabled("service")) {
			...
			echo '<input type="hidden" class="prod_entry_mode_predef" name="prod_entry_mode" id="prod_entry_mode_predef" value="predef"'.(GETPOST('prod_entry_mode') == 'predef' ? ' checked' : '').'> ';
			echo '</label>';
			echo ' ';
			$filtertype = '';
			...
			if (empty($senderissupplier)) {
				...
				if (!empty($conf->global->ENTREPOT_EXTRA_STATUS)) {
					...
					$form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, $statustoshow, 2, '', 1, array(), $buyer->id, '1', 0, '', 0, $statuswarehouse, GETPOST('combinations', 'array'));
				} else {
					$form->select_produits(GETPOST('idprod'), 'idprod', $filtertype, $conf->product->limit_size, $buyer->price_level, $statustoshow, 2, '', 1, array(), $buyer->id, '1', 0, '', 0, '', GETPOST('combinations', 'array'));
				}
				...
				$form->select_produits_fournisseurs($object->socid, GETPOST('idprodfournprice'), 'idprodfournprice', '', '', $ajaxoptions, 1, $alsoproductwithnosupplierprice, 'fullwidth',$langs->trans("ChooseProductPlaceholder"));
	...
	<?php if($object->status!=RequestPurchaseOrder::STATUS_DRAFT && $object->status!=RequestPurchaseOrder::STATUS_REJECTED){ ?>
