...
/**
 *  \file       requestpurchaseorderline_agenda.php
 *  \ingroup    requestorderwithoutvendor
 *  \brief      Tab of events on RequestPurchaseOrderLine
 */
...
$hookmanager->initHooks(array('requestpurchaseorderlineagenda', 'globalcard')); // Note that conf->hooks_modules contains array
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->requestorderwithoutvendor->multidir_output[!empty($object->entity) ? $object->entity : $conf->entity]."/".$object->id;
}
...
if ($object->id > 0) {
	...
	print dol_get_fiche_head($head, 'agenda', $langs->trans("RequestPurchaseOrderLine"), -1, $object->picto);
	...
	if (isModEnabled('agenda') && (!empty($user->rights->agenda->myactions->read) || !empty($user->rights->agenda->allactions->read))) {
		...
		//print load_fiche_titre($langs->trans("ActionsOnRequestPurchaseOrderLine"), '', '');
		print_barre_liste($langs->trans("ActionsOnRequestPurchaseOrderLine"), 0, $_SERVER["PHP_SELF"], '', $sortfield, $sortorder, '', 0, -1, '', 0, $morehtmlright, '', 0, 1, 1);
