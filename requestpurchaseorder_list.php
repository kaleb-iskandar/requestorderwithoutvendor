...
/**
 *   	\file       requestpurchaseorder_list.php
 *		\ingroup    requestorderwithoutvendor
 *		\brief      List page for requestpurchaseorder
 */
...
require_once __DIR__.'/class/requestpurchaseorder.class.php';
...
$hookmanager->initHooks(array('requestpurchaseorderlist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('requestorderwithoutvendor', 'requestpurchaseorder', 'write');
	$permissiontodelete = $user->hasRight('requestorderwithoutvendor', 'requestpurchaseorder', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'RequestPurchaseOrder';
	$objectlabel = 'RequestPurchaseOrder';
...
//$help_url = "EN:Module_RequestPurchaseOrder|FR:Module_RequestPurchaseOrder_FR|ES:Módulo_RequestPurchaseOrder";
$help_url = '';
$title = $langs->trans("RequestPurchaseOrders");
$morejs = array();
$morecss = array();
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/requestorderwithoutvendor/requestpurchaseorder_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/requestorderwithoutvendor/requestpurchaseorder_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendRequestPurchaseOrderRef";
$modelmail = "requestpurchaseorder";
$objecttmp = new RequestPurchaseOrder($db);
